...
import { createMeshyImageTo3dTask, getMeshyImageTo3dTask, isMeshyConfigured } from "@/lib/ai3d";
...
export async function GET(request: Request) {
  ...
  try {
    const task = await getMeshyImageTo3dTask(id);
    return NextResponse.json({ provider: "meshy", task });
  } catch (error) {
